<template>
	<div class="page" id="onePrivateWeekBox">
        <div class="page-content">
            <div class="week">
                <div class="kong"></div>
                <div v-for="date in allList">
                    <span>{{date.date}}</span>
                    <span>{{date.time}}</span>
                </div>
            </div>
            <div class="class-tab">
                ...
                <div class="orderListBox">
                    <ul v-for="date in allList">
                       <li v-for="order in date.timeList" 
                                :class="{hasUser:order.state==0,
                                        noContent:order.state==1,
                                        teacherRest:order.state==2}" 
                                @click="message(order.state,date.timeList,date.time,date.date)"
                        >
    ...
    import {Popup} from 'mint-ui';
  	export default {
        ...
        methods:{
            getDate(){
                var _this = this;
                this.$http.get('/src/assets/js/private.json')
                .then(function (data) {
                    var value = data.data.week;
                    for(var i in value){
                        _this.allList.push(value[i]);
                    }
                })
                .catch(function (data) {
                    // alert(data);
