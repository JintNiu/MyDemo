            ...
            <div class="contentTable">
                <div class="tableListContent"
                    ...
                    <div class="userContentBox" id="userContentBox">
                        <ul v-for="item in list" :id="item.userId" @click="getMainContent(item.userId)">
                            <i>
                            ...
                            <ul class="userContent">
                                ...
                                <li>
                                    <span class="memberName">{{item.userName}}
                                        <span style="float:right">(教练：{{item.teacherName}})</span>
                                    </span>
                                    <span class="cardTime">{{item.createTime}}</span>
                                </li>
                            </ul>
                            <b class="fa fa-angle-right"></b>
                        </ul>
                    </div>
                    <mt-spinner type="snake" style="width:1rem;height:1rem;margin:0 auto;display:block;" v-if="moreLoading&&!noMore"></mt-spinner>
...
<script>
    import {Spinner,MessageBox,InfiniteScroll} from 'mint-ui';
    export default {
        ...
        methods:{
            ...
            //         // this.moreLoading = true; // 测试loading样式
            //     });
            // },
            ...
            contentList(){ // 初始化数据
                var num = 0;
                var _this = this;
                this.$http.get('/LuchyBird/manage/publicClassByPage.do?clubId='+_this.clubId+'&state=2&page=1&pageSize='+this.pageSize+'&startTime='+this.allStartTime+'&endTime='+this.allEndTime+'').then(function(data){
            ...
            dateUpdateData(startTime,endTime){ // 改变日期对应改变数据
                var _this = this;
                this.$http.get('/LuchyBird/manage/publicClassByPage.do?clubId='+this.clubId+'&state=2&page=1&pageSize='+this.pageSize+'&startTime='+startTime+'&endTime='+endTime+'').then(function(data){
            ...
            loadMore() { // 加载更多
                ...
                // setTimeout(()=>{
                    this.page ++;
                    this.$http.get('/LuchyBird/manage/publicClassByPage.do?clubId='+_this.clubId+'&state=2&page='+this.page+'&pageSize='+this.pageSize+'&startTime='+this.allStartTime+'&endTime='+this.allEndTime+'').then(function(data){
                        var value = data.data.data;
                        if(_this.list.length == 0){ // 如果初始没有数据不继续加载
                            _this.noMore = false;
