<template>
    <div class="page" id="courseRecord">
       <mt-header fixed title="课程记录" style="background:#000">
            <mt-button slot="left" icon="back" @click="coursePopup()"></mt-button>
        </mt-header>
        <div class="courseList" id="courseList" @scroll.native="woListScroll($event)">
            <ul>
                <li v-for="course in courseRecordList">
                    <i>
...
export default {
    ...
    mounted (){
        ...
        this.getCourseList();
        document.getElementById('courseList').addEventListener('scroll', this.woListScroll);
    },
    methods:{
        // coursePopup(){
        //     this.$emit('closeCoursePopup',false); // 自定义事件，在父级调用closeCardPopup
        // }
        woListScroll() {
            this.scrollTop = window.scrollY + window.screen.availHeight - document.getElementById('courseList').offsetHeight - 80;
            if(this.scrollTop >= 0){
        ...
        getCourseList(){
            let num = 0;
            let _this = this;
            this.$http.get('/LuchyBird/publicUser/getUserClassDetail.do?userId='+this.userId+'&isPersonal=0&currentPage=1&pageSize='+this.pageSize+'').then(function(data){
        ...
        loadMore() { // 加载更多
            ...
            this.currentPage ++; 
            this.$http.post('/LuchyBird/manage/getUserClassDetail.do?userId='+this.userId+'&isPersonal=0&currentPage='+this.currentPage+'&pageSize='+this.pageSize+'').then(function(data){
                let value = data.data.data;
                if(_this.courseRecordList.length == 0){ // 如果初始没有数据不继续加载
                    _this.noMore = false;
                    ...
                    if(value == '' || value == null || value.length == 0){
                        ...
                        for(let i in value){
                            num ++;
                            _this.courseRecordList.push(value[i]);
                        }
                        if(num<_this.pageSize && num != 0){ // 如果初始数据小于初始默认加载数据的条数，没有更多提示显示
                            _this.noMore = true;
                        }else{
